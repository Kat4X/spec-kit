...
Не используй старые `ai/specs/*`, `plan.md` или `tasks.md` как источник требований для этого change, если пользователь явно не указал связанный workspace.
...
Пиши plan так, чтобы следующий AI мог сгенерировать `tasks.md` без доступа к этому диалогу.
