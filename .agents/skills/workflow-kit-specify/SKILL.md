...
description: "Используй когда пользователь просит создать/уточнить спецификацию Workflow Kit для одного atomic change: зафиксировать WHAT/WHY, desired result, boundaries, out-of-scope, проверки и открытые вопросы перед техническим планом. Не использовать для реализации кода."
---
...
Перед созданием файлов проверь доступность `assets/SPEC-TEMPLATE.md`. Если шаблон недоступен — остановись и сообщи, что отсутствует обязательный шаблон; workspace не создавай.
...
`assets/SPEC-TEMPLATE.md` — единственный источник структуры `spec.md`. Не создавай структуру из памяти и не используй резервные шаблоны.
...
- [ ] Верхнеуровневый формат соответствует `assets/SPEC-TEMPLATE.md`.
