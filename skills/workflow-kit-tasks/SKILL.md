...
- `execution.parallelGroups` заполняй массивами task IDs, если есть независимые группы.
...
- Если пользователь попросил подготовить commit, добавляй только не ignored workflow artifacts (`tasks.json`, `verification.md`) и сначала проверь `.gitignore`.
